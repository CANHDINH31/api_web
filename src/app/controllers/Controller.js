...
import FunlearingModel from "../models/Funlearing.js";
...
export const RenderCategory = (req, res, next) => {
  ...
  Promise.all([
    DetailModel.countDocuments({ code: category }),
    DetailModel.find({ code: category }).lean(),
  ])
    .then(([count, images]) => {
...
export const SaveApiDetailsCreate = (req, res, next) => {
  ...
  ApiDetail.save().then(res.redirect("back")).catch(next);
};

export const SaveApiCategoryCreate = (req, res, next) => {
  const ApiCategory = new MenuModel(req.body);
  ApiCategory.save().then(res.redirect("back")).catch(next);
};
...
export const UpdateApiCategory = (req, res, next) => {
  MenuModel.find({ _id: req.params.id })
    .lean()
    .then((apicategory) =>
      res.render("UpdateApiCategory", {
        apicategory,
      })
    )
    .catch((error) => {
...
export const VideoStore = (req, res, next) => {
  Promise.all([
    VideoModel.find({ param: "nhactrung" })
      .sort("name")
      .skip(0)
      .limit(8)
      .lean(),
    VideoModel.find({ param: "nhacviet" }).sort("name").skip(0).limit(8).lean(),
    VideoModel.find({ param: "nhacquocte" })
      .sort("name")
      .skip(0)
      .limit(8)
      .lean(),
    VideoModel.find({ param: "tuyenvietnam" })
      .sort("name")
      .skip(0)
      .limit(8)
      .lean(),
...
export const VideoDetail = (req, res, next) => {
  ...
  Promise.all([
    VideoModel.find({ param: param, _id: id }).sort("name").lean(),
    VideoModel.find({ param: param }).sort("name").lean(),
    VideoModel.find({}).select("type"),
  ])
    .then(([videodetail, listvideodetail, listtype]) => {
  ...
  VideoModel.find({})
    ...
    .then((results) => {
      ...
      results = results.filter((result) => {
        return result.name.includes(string) || result.type.includes(string);
      });
      const videoid = [];
      results.map((result) => {
        videoid.push(result.videoid);
      });
      Promise.all([
        VideoModel.find({ videoid: { $in: videoid } })
          .sort("name")
          .lean(),
        VideoModel.countDocuments({ videoid: { $in: videoid } }),
      ])
        .then(([data, count]) =>
...
export const ApiFunLearingBackGround = (req, res, next) => {
  const { type } = req.params;
  FunlearingModel.find({ type: type })
    .then((result) => {
      res.json(result);
    })
    .catch((e) => {
